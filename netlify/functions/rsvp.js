...
export const handler = async (event) => {
  ...
  const toEmail = "mouko.zeijnet@icloud.com";
  ...
  const submittedAt = new Date().toLocaleString("sr-Latn-RS", {
    dateStyle: "medium",
    timeStyle: "short",
    timeZone: "Europe/Skopje",
  });

  const isAttending = attendance === "Da";
  const attendanceColor = isAttending ? "#7b8a5f" : "#9a6b52";
  const attendanceLabel = isAttending ? "Dolaze" : "Ne dolaze";
  const attendanceText = isAttending ? "Da, sa zadovoljstvom" : "Nažalost, nisu u mogućnosti";
    ...
    <div style="margin:0; padding:28px 16px; background:#f7f4ef; font-family:Georgia, 'Times New Roman', serif; color:#593b00;">
      <div style="max-width:620px; margin:0 auto; background:#fffdf8; border:1px solid #e8ddc9; border-radius:10px; overflow:hidden;">
        <div style="padding:30px 30px 24px; background:#fbfaf5; border-bottom:1px solid #e8ddc9; text-align:center;">
          <p style="margin:0 0 10px; color:#c4a77b; font-family:Arial, Helvetica, sans-serif; font-size:12px; letter-spacing:2.2px; text-transform:uppercase;">Deyla &amp; Ervin</p>
          <h1 style="margin:0; color:#493d33; font-size:30px; font-weight:400; line-height:1.15;">Novi odgovor za venčanje</h1>
          <p style="margin:12px 0 0; color:#826f5d; font-size:16px;">26.09.2026.</p>
        </div>
        ...
        <div style="padding:28px 30px 10px;">
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:22px; border-collapse:collapse;">
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Ime i prezime</td>
            </tr>
            <tr>
              <td style="padding:0 0 16px; font-size:26px; color:#593b00; line-height:1.2;">${escapeHtml(name)}</td>
            </tr>
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Dolazak</td>
            </tr>
            <tr>
              <td style="padding:0 0 16px; font-size:18px; line-height:1.35;">${escapeHtml(attendanceText)}</td>
            </tr>
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Broj osoba u pratnji</td>
            </tr>
            <tr>
              <td style="padding:0 0 16px; font-size:18px;">${escapeHtml(guestCount)}</td>
            </tr>
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Imena osoba u pratnji</td>
            </tr>
            <tr>
        ...
        <div style="padding:18px 30px 24px; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:13px; background:#fbf8f1; border-top:1px solid #eee6d8;">
          Poslato: ${escapeHtml(submittedAt)}
        </div>
      </div>
    </div>
  `;
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: fromEmail,
      to: [toEmail],
      subject: `Novi odgovor za venčanje: ${name} — ${attendanceLabel}`,
      html,
    }),
  });
