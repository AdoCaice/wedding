...
export const handler = async (event) => {
  ...
  const attendanceLabel = isAttending ? "Doagjaat" : "Ne doagjaat";
  const attendanceText = isAttending ? "Da, so zadovolstvo" : "Ne, za zhal ne sum vo mozhnost";
    ...
    <div style="margin:0; padding:28px 16px; background:#f7f4ef; font-family:Georgia, 'Times New Roman', serif; color:#593b00;">
      <div style="max-width:620px; margin:0 auto; background:#fffdf8; border:1px solid #e8ddc9; border-radius:10px; overflow:hidden;">
        <div style="padding:30px 30px 24px; background:#fbfaf5; border-bottom:1px solid #e8ddc9; text-align:center;">
          ...
          <h1 style="margin:0; color:#593b00; font-size:30px; font-weight:400; line-height:1.15;">Nov odgovor za svadbata</h1>
        ...
        <div style="padding:28px 30px 10px;">
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:22px; border-collapse:collapse;">
            ...
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Prisustvo</td>
            </tr>
            <tr>
              <td style="padding:0 0 16px; font-size:18px; line-height:1.35;">${escapeHtml(attendanceText)}</td>
            </tr>
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Broj na pridruzhba</td>
            </tr>
            <tr>
              <td style="padding:0 0 16px; font-size:18px;">${escapeHtml(guestCount)}</td>
            </tr>
            <tr>
              <td style="padding:16px 0 6px; border-top:1px solid #eee6d8; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:12px; text-transform:uppercase; letter-spacing:1.4px;">Iminja na pridruzhba</td>
            </tr>
            <tr>
        ...
        <div style="padding:18px 30px 24px; color:#8a7653; font-family:Arial, Helvetica, sans-serif; font-size:13px; background:#fbf8f1; border-top:1px solid #eee6d8;">
          Isprateno: ${escapeHtml(submittedAt)}
        </div>
      </div>
    </div>
  `;
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: fromEmail,
      to: [toEmail],
      subject: `Nov odgovor za svadbata: ${name} - ${attendanceLabel}`,
      html,
    }),
  });
